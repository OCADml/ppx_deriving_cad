...
Alteratively to the below, odoc generated documentation is also available
[online](https://ocadml.github.io/ppx_deriving_cad/ppx_deriving_cad/Ppx_deriving_cad/index.html).
...
val xscale_mark : float -> t -> t
val yscale_mark : float -> t -> t
val zscale_mark : float -> t -> t
...
derived) will be given unqualified names. For example, applying `[@@deriving cad]`
...
By default, `[@@deriving cad]` will attempt to map over constructors other than
...
type vec_map = V3.t IntMap.t [@@deriving cad]
```
...
with mixed mapping conventions, you can make use of the [`[@cad.map]` and
`[@cad.mapf]`](#cadmap-and-cadmapf) attributes to specify fields that
...
`OSCADml.Scad.t` type. Of course, when the type could be either **2d** or **3d**, only
...
Thus, `[@cad.map]` indicates that relevant `map` functions will obey the
convention of `f` being the first *positional* argument (overiding `[@@deriving
cad_jane]`), whereas `[@cad.mapf]` indicates that a keyword argument of `~f`
is expected instead (overiding `[@@deriving cad]`). These attributes are not
...
concretely dimensional types such as `Scad.d3`, or `V2.t`), these annotations
should be used to specify the correct set of functions/signatures to be
generated.
...
`[@@deriving cad]` into whether it is **2d** or **3d**, so we tag on an attribute
to clear it up.
